 ...
 * or at least part of one.  Abstracts out a lot of common logic, and makes
 * it easy to send responses to requests.
 *
...
public abstract class JsonRestServlet extends HttpServlet {
    ...
    protected void sendResponse(HttpServletRequest req, HttpServletResponse resp,
        ...
        try {
            ...
            sendError(resp, 500, new StringBuffer()
    ...
    protected void checkAuthorization(HttpServletRequest req)
        ...
        String currentUserId = req.getSession()
     ...
     * @throws GoogleTokenExpirationException User's token is expired and cannot
     *                                        be used.
     */
    protected GoogleCredential getCredentialFromLoggedInUser
    (HttpServletRequest req) throws GoogleTokenExpirationException {
        User loggedInUser = ofy().load().type(User.class).id(Long.parseLong(
                req.getSession().getAttribute(CURRENT_USER_SESSION_KEY).toString())).get();
